...
on dopreview()
	...
	set endline to " 2>> " & tmpdir & "songredux_time"
	set preview to true
	set quotedoutputfile to "/tmp/ahtemp/ahpreview" & fullstarttime
	...
	if contents of button "previewcomp" of window "preview" is true and (contents of popup button "formats" of box "box" of window "SongRedux" is not in {wav, aiff, flac, applelossless, audiocd}) then
	...
	do shell script "cd /tmp ; " & nice & "/bin/sh -c \"" & ffmpegstring & endline & "\" &> /dev/null ; exit 0"
	
	
	--FIX QTCANTREADS
	if toformat is in {wma, ogg, flac} then
		do shell script "cd /tmp ; " & nice & "/bin/sh -c \"" & ffmpeg & " -y -i " & " /tmp/ahtemp/ahpreview" & fullstarttime & ".temp." & fileext & " /tmp/ahtemp/ahpreview.temp.wav \" &> /dev/null ; exit 0"
		set fileext to "wav"
		do shell script "/bin/rm -f /tmp/ahtemp/ahpreview" & fullstarttime & ".temp." & fileext & " ; /bin/mv /tmp/ahtemp/ahpreview.temp.wav /tmp/ahtemp/ahpreview" & fullstarttime & ".temp." & fileext & " ; exit 0"
	end if
	
	tell window "preview"
		set moviefile to "/tmp/ahtemp/ahpreview" & fullstarttime & ".temp." & fileext
		try
		...
		on error
			...
			--do shell script "open /tmp/ahtemp"
		end try
		update
	end tell
end dopreview