...
on AddFiles(theFiles)
	...
	do shell script "/usr/bin/open -b com.techspansion.songredux " & readyfiles & " ; exit 0"
	-- WAIT UNTIL DONE
	set howmanyrows to 0
	set thismanyrows to 1
	repeat until howmanyrows is thismanyrows
		tell application "SongRedux" to set howmanyrows to number of data rows of data source of table view "table" of scroll view "table" of window "SongRedux"
		delay 1.5
		tell application "SongRedux" to set thismanyrows to number of data rows of data source of table view "table" of scroll view "table" of window "SongRedux"
	end repeat
	return true
end AddFiles
...
on clearall()
	tell application "SongRedux"
		delete every data row of data source of table view "table" of scroll view "table" of window "SongRedux"
...
on SetSaveLocation(saveas)
	if saveas is null then
		tell application "SongRedux" to set content of text field "saveto" of box "workflowbox" of box "box" of window "SongRedux" to ""
	else
		set AppleScript's text item delimiters to ""
		...
		tell application "SongRedux"
			set content of text field "saveto" of box "workflowbox" of box "box" of window "SongRedux" to saveasposix
...
on StartConversion()
	...
	tell application "SongRedux"
		...
		if contents of text field "supper" of window "SongRedux" is "" then
			set contents of text field "supper" of window "SongRedux" to "auto"
		end if
		...
		if whatstatus is in {(localized string "addandclickstarttostart"), (localized string "clickstarttostart"), ""} then
			call method "performClick:" of object (button "start" of window "SongRedux")
		end if
		--		with timeout of 35000000 seconds
		--			set size of text field "supper" of window "SongRedux" to {stupidhack, 1}
		--		end timeout
	end tell
end StartConversion
...
on StartAssemblyLine()
	--	set stupidhack to (do shell script "echo $RANDOM | /usr/bin/cut -c 1-3")
	tell application "SongRedux"
		if contents of text field "timeremaining" of window "SongRedux" is in {(localized string "addandclickstarttostart"), (localized string "clickstarttostart"), ""} then
			--	doitmulti("auto") of (load script (scripts path of main bundle & "/main.scpt" as POSIX file))
			set whatstatus to contents of text field "timeremaining" of window "SongRedux"
			if whatstatus is in {(localized string "addandclickstarttostart"), (localized string "clickstarttostart"), ""} then
				set contents of text field "wanter" of window "SongRedux" to "assembly"
				do shell script "/usr/bin/osascript -e 'tell application \"SongRedux\" to call method \"performClick:\" of object (button \"start\" of window \"SongRedux\")' &> /dev/null & "
			end if
			--		with timeout of 35000000 seconds
			--			set size of text field "supper" of window "SongRedux" to {stupidhack, 1}
			--		end timeout
...
on loadsettings(theFile)
	...
	if text item 1 of (theFile as string) is "/" then
		do shell script "/usr/bin/open -b com.techspansion.songredux " & (quoted form of theFile)
	else
		do shell script "/usr/bin/open -b com.techspansion.songredux " & (quoted form of POSIX path of theFile)
	end if
end loadsettings

on QuitApp()
	tell application "SongRedux" to quit
end QuitApp

on ForceQuit()
	tell application "SongRedux" to quit saving no
end ForceQuit
...
on SetWhenDone(theoption)
	try
		...
		if theoption < 5 then
			set contents of popup button "postaction" of box "workflowbox" of box "box" of window "SongRedux" to (theoption - 1)
		end if
	on error
		...
		tell application "SongRedux"
			...
			set contents of popup button "postaction" of box "workflowbox" of box "box" of window "SongRedux" to 5
			update window "SongRedux"
		end tell
	end try
end SetWhenDone
...
on TurnXgridOn(saveas)
	tell application "SongRedux"
		set content of button "Xgrid" of window "SongRedux" to true
		try
			set AppleScript's text item delimiters to ""
			if text item 1 of (saveas as string) is "/" then
				set saveasposix to (saveas as Unicode text)
			...
			tell application "SongRedux" to set content of text field "path" of window "SongRedux" to saveasposix as Unicode text
		on error
...
on TurnXgridOff()
	tell application "SongRedux" to set content of button "Xgrid" of window "SongRedux" to false
end TurnXgridOff
*)
